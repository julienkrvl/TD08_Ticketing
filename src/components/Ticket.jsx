		...
		if (this.event !== null && typeof this.props.contracts['OpenEvents'].getEvent[this.event] !== 'undefined') {
			...
			if (this.state.card_tab === 1) {
				...
				card_body =
					<div>
						<div className="card-body">
							...
							{timeStatus}
							<h5 className="text-center mb-0">Votre place: {ticket_data[1]}</h5>
						</div>
						...
						<div className="card-footer text-muted event-header">
							...
							<p className="small text-truncate mb-2">
								Créateur: <a href={"https://rinkeby.etherscan.io/address/" + event_data[8]} target="_blank">
									{event_data[8]}
								</a>
							</p>
						</div>
				...
				card_body =
					<div>
						<div className="card-body">
							<h5 className="text-center">Envoyer le ticket :</h5>
					...
					<div className="card-header">
						<ul className="nav nav-tabs card-header-tabs">
							<li className="nav-item">
								<a href="" className={"nav-link " + (this.state.card_tab === 1 ? 'active' : '')} onClick={(e) => this.changeTab(1, e)}>A propos de l'évenement</a>
							</li>
							<li className="nav-item">
								<a href="" className={"nav-link " + (this.state.card_tab === 2 ? 'active' : '')} onClick={(e) => this.changeTab(2, e)}>Transférer ce ticket</a>
							</li>
						</ul>
					</div>
					{card_body}
